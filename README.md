...
| `ORQ_SERVER` | API host, for self-hosted deployments. Unset, the host is whatever the orq CLI reports (`orq server set`, the login session, else `my.orq.ai`). `ORQ_API_BASE_URL` is the deprecated spelling and still honored |
| `ORQ_WORKSPACE` | Pin the workspace for this run, overriding the session's active one. No effect with `ORQ_API_KEY`, which names its own workspace |
...
| `ORQI_AGENT_DIR`, `ORQ_MCP_URL`, `ORQ_GATEWAY_URL` | Override the agent dir, or pin individual endpoints (on-prem) |
